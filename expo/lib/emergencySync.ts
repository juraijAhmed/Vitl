...
export async function syncEmergencyProfile(
  ...
  const emergencyId =
    profile.emergencyId ?? `v-${Math.random().toString(36).substring(2, 8)}`;
  ...
  const { error } = await supabase.from("emergency_profiles").upsert(
    {
      ...
      emergency_contact_name: profile.emergencyContacts[0]?.name ?? "",
      emergency_contact_phone: profile.emergencyContacts[0]?.phone ?? "",
    },
    {
